...
async def game_finnished(message) -> bool:
	...
	if players_list_keys:
		...
		for i in players_list_keys:
			check_player = cur.execute("SELECT * FROM players WHERE id = ?;", [i]).fetchall()
			print(str(check_player[0]).split(', ')[1][:-1])
			if check_player:
				cur.execute("UPDATE players set score = ? WHERE id = ?;", [players_list[i] + int(str(check_player[0]).split(', ')[1][:-1]), i])
